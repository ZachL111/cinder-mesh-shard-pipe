...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-cinder-mesh-shard-detail.ps1

sqlite3 :memory: ".read tests/domain_review.sql"

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-cinder-mesh-shard-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-cinder-mesh-shard-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-cinder_mesh_shard_pipe-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
